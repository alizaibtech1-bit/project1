...
router.post('/:id/reviews', async (req, res) => {
  try {
    ...
    if (!name || !rating || !comment) return res.status(400).json({ message: 'Name, rating, and comment are required' });
    const review = await Review.create({ product: req.params.id, name, rating, title, comment, isVerified: false });
    ...
    await Product.findByIdAndUpdate(req.params.id, { rating: parseFloat(avgRating), numReviews: allReviews.length });
